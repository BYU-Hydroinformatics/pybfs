...
def bfs_calibrate_beta1(tmp_site, tmp_area, tmp_q, dys):
    """Calibrate a site
    ...
    # ============================================================
    # STEP 2: FORCE BETA = 1 AND CALIBRATE BASE + SURFACE
    # ============================================================
    print("  Step 2: Calibrating baseflow function at forced beta=1...")

    # ---- Force beta ----
    b = 1.0
    BETA = 1.0

    # ---- Initialize X1 using the same relationship used in your loop ----
    ...
    # ------------------------------------------------------------
    # 2A) Optimize basetable-related params ONCE (no beta sweep)
    #     X = [Lb, X1, Wb, Kb]
    # ------------------------------------------------------------
    if np.isfinite(X1) and X1 > 0:
        X = np.array([Lb, X1, Wb, Kb], dtype=float)

        print("    Optimizing basetable parameters (Lb, X1, Wb, Kb) at beta=1.0...")
        result_bt = _create_scaled_minimize(
            cal_basetable,
            X,
            X,  # parscale = X
        ...
        # Guard: if optimization returns non-finite, keep previous values
        if np.all(np.isfinite(result_bt.x)):
            Lb = float(result_bt.x[0])
            X1 = float(result_bt.x[1])
            Wb = float(result_bt.x[2])
            Kb = float(result_bt.x[3])
            print(f"    Basetable objective={result_bt.fun:.6f} | Lb={Lb:.6f}, X1={X1:.6f}, Wb={Wb:.6f}, Kb={Kb:.6f}")
        else:
            print("    WARNING: Basetable optimization returned non-finite values; keeping existing Lb, X1, Wb, Kb.")

    # Basin/gw structures (match your existing design)
    basin_char = [tmp_area, Lb, X1, Wb, POR]
    gw_hyd = [ALPHA, BETA, Ks, Kb, Kz]

    # ------------------------------------------------------------
    # 2B) Optimize base parameters (Lb, Wb, Kb, Kz)
    # ------------------------------------------------------------
    X = np.array([Lb, Wb, Kb, Kz], dtype=float)

    print("    Optimizing base parameters (Lb, Wb, Kb, Kz)...")
    result = _create_scaled_minimize(
        cal_base,
        X,
        X,  # parscale = X
    ...
    print(
        f"    Optimization success: {result.success}, iterations: {getattr(result, 'nit', np.nan)}, final objective: {result.fun:.6f}")

    # IMPORTANT: avoid killing the run due to tiny negative steps; clamp instead of NaN
    eps = 1e-12
    if np.all(np.isfinite(result.x)):
        # Lb, Wb, Kb, Kz should be > 0
        result.x[0] = max(float(result.x[0]), eps)
        result.x[1] = max(float(result.x[1]), eps)
        result.x[2] = max(float(result.x[2]), eps)
        result.x[3] = max(float(result.x[3]), eps)

    if np.all(np.isfinite(result.x)):
        print(
            f"    Optimized params: Lb={result.x[0]:.6f}, Wb={result.x[1]:.6f}, Kb={result.x[2]:.6f}, Kz={result.x[3]:.6f}")
    ...
    if np.all(np.isfinite(result.x)):
        ...
        # keep basin_char/gw_hyd consistent before cal_surface
        basin_char = [tmp_area, Lb, X1, Wb, POR]
        gw_hyd = [ALPHA, BETA, Ks, Kb, Kz]

        # ------------------------------------------------------------
        # 2C) Optimize surface parameters (Lb, Wb, ALPHA, Ks) in log10-space
        # ------------------------------------------------------------
        X = np.array([Lb, Wb, ALPHA, Ks], dtype=float)
        ...
        print("    Optimizing surface parameters (Lb, Wb, ALPHA, Ks) in log10-space...")
        result_surf = _create_scaled_minimize(
            wrapped_cal_surface,
            LOGX,
            LOGX,  # parscale = LOGX
            streamflow_df, 'day', 'base', basin_char, gw_hyd, flow,
        ...
        print(
            f"    Optimization success: {result_surf.success}, iterations: {getattr(result_surf, 'nit', np.nan)}, final objective: {result_surf.fun:.6f}")
        if np.all(np.isfinite(result_surf.x)):
            print(
                f"    Optimized params: Lb={10 ** result_surf.x[0]:.6f}, Wb={10 ** result_surf.x[1]:.6f}, "
                f"ALPHA={10 ** result_surf.x[2]:.6f}, Ks={10 ** result_surf.x[3]:.6f}"
            )

        # Update params only if cal_surface succeeded with finite x (match your existing behavior)
        if np.all(np.isfinite(result_surf.x)):
            Lb = float(10 ** result_surf.x[0])
            Wb = float(10 ** result_surf.x[1])
            ALPHA = float(10 ** result_surf.x[2])
            Ks = float(10 ** result_surf.x[3])
        ...
        # ------------------------------------------------------------
        # 2D) Run BFS + compute errors + append bf_params (even if cal_surface failed)
        # ------------------------------------------------------------
        SBT = base_table(
            basin_char[1],  # Lb
            basin_char[2],  # X1
        ...
        den = np.nansum(bfs_out[qob_col][~np.isnan(bfs_out[qob_col])])
        num = np.nansum(tmp_bf[~np.isnan(bfs_out[qob_col])])
        BFF = (num / den) if den > 0 else np.nan
        ...
        print("    WARNING: cal_base returned non-finite parameters; skipping cal_surface + BFS for Step 2.")
